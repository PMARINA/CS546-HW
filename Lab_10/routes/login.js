...
router.post("/", authMiddleware, async (req, res) => {
  if (req.hasBeenCookieAuthenticated) {
    ...
    if (!pwdHelp && !unameHelp && !genHelp) {
      ...
      try {
        if (!unameHelp && !(await UserDB.exists({ username: uname.toLowerCase() }))) {
          genHelp = true;
        }
        if (!unameHelp) {
          result = await Users.checkUser(uname, pd);
    ...
    if (unameHelp || pwdHelp || genHelp) {
      ...
      if (genHelp === true){
          genHelp = "The username and/or password were incorrect."
      }
      res
        .status(statusCode)
        .render("login", {
